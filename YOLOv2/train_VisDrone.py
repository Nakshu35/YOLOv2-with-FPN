...
from Data.config_VisDrone import S, B, C, Inp_size, Epochs, Batch_size, Lr, Wd
from Data.Target_builder import YOLO_TargetBuilder
from Data.anchor_boxes import extract_box_wh, kmeans

from Loss.loss_VisDrone import YOLO_loss
...
VisDrone_train = VisDroneDataset(
    root=root_path,
    split="train",
    transform=transform
)

VisDrone_val = VisDroneDataset(
    root=root_path,
    split="val",
    transform=transform
)
...
#anchors
boxes = np.array(extract_box_wh(VisDrone_train, Inp_size))
anchors = kmeans(boxes, k=B)
...
checkpoint_dir = "checkpoints\\YOLOv2_VisDrone"
